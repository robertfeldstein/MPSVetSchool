...
predictors24 <- c("PIPActive", "time","CastrationPerformed", "OvPerformed", "Hosp", "Early")
target_vars<- c("medical", "clinical_reasoning", "professionalism", "collaboration", "procedural")
df24 <- df24[!is.na(df24$procedural), ]
#split data into train and test sets
train_index24 = createDataPartition(y = df24$procedural, p = 0.8, list = FALSE)
train24<- df24[train_index,]
test24<- df24[-train_index,]
#define k-fold cross validation
control24<- trainControl(method = "cv", number = 5, verboseIter = FALSE)
